...
async function initEngine() {
    ...
    // Get API key from localStorage, input, or fetch from server
    let apiKey = localStorage.getItem('vinylVibeApiKey');
    
    // If no API key in localStorage, try to fetch from server
    if (!apiKey) {
        // Attempt to fetch the API key from our backend
        fetch('/api/config')
            ...
            .then(data => {
                if (data && data.openaiApiKey) {
                    ...
                    // Initialize engine with the retrieved key
                    initEngineWithKey(apiKey);
                }
            })
            .catch(error => {
                console.error('Failed to fetch API key:', error);
                log('Failed to fetch API key. Please enter one manually.');
            });
    }
